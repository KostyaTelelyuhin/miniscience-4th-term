...
int main(int argc, char* argv[])
{
...

  {
    ...
    //auto mesh = std::make_shared<mesh::Mesh>(mesh::create_disk(MPI_COMM_WORLD, 0.5, 0.5, 1.0, 8, 32));
    ...
    f->interpolate(
        [](auto&& x) -> xt::xarray<T>
        {
          ...
          return 5000 * xt::exp(-(dx) / 100);
        });

    //g->interpolate([](auto&& x) -> xt::xarray<T>
    //              { return xt::zeros_like(xt::row(x, 0)); });
    
     
                 
      g->interpolate([](auto&& x) -> xt::xarray<T>
                   { return xt::exp(5 * xt::row(x, 0)); });
